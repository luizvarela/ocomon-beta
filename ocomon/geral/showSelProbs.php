<?php /*                        Copyright 2005 Flï¿½vio Ribeiro
  ...
  */session_start();
	...
	if (isset($_GET['pathAdmin'])) { //se o script estiver sendo chamado a partir do path do mï¿½dulo de administraï¿½ï¿½o
	...
	if($_GET['area_habilitada']=='sim'){
		if($_GET['area_cod']=="" || $_GET['area_cod']==-1){
			...
			while ($row_prob = mysql_fetch_array($exec_prob)) {
				...
				/*
				 * Alteração TECSOFT : Sandro
				 * 
				 * Verificar se o valor da lista de problemas é igual a algum valor dos radiobox
				 *   
				 * */				 
				$queryTmp = "SELECT problema,prob_id FROM problemas WHERE problema= '".$row_prob['problema']."'";
				 
				$exec_probTmp = mysql_query($queryTmp);
				while ( $row_probTmp = mysql_fetch_array($exec_probTmp) ) 
				{ 
					//if ($row_probTmp['prob_id'] == $_GET['prob'] ) print " selected";
					if ($row_probTmp['prob_id'] == $_GET['prob'] ) print " selected";
					//print $queryTmp;
				}
				 /*
				 * Fim alteração TecSoft:Sandro
				 * 
				 * */				 
