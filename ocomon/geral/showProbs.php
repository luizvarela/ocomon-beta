  ...
  */session_start();
	...
	$auth->testa_user_hidden($_SESSION['s_usuario'],$_SESSION['s_nivel'],$_SESSION['s_nivel_desc'],4);
			...
			print "<TABLE border='0' cellpadding='2' cellspacing='0' width='90%'>";
				...
				$queryInfProbs = "SELECT * FROM problemas as p ".
				...
				if (isset($_GET['prob']) && $_GET['prob'] != -1 )  { //&& $_POST['problema'])
					$queryInfProbs.= " WHERE lower(p.problema) like lower(('%".$rowId['problema']."%')) ".$clausula."";
				} else
					$queryInfProbs.= " WHERE p.problema = -1 ".$clausula."";


				$queryInfProbs .=" ORDER  BY s.sistema, p.problema";
				
				//print $queryInfProbs;
				$resultado = mysql_query($queryInfProbs) or die(TRANS('ERR_QUERY'));
				...
				else
				{
					...
					$j=2;
					while ($rowInfProb = mysql_fetch_array($resultado))
					{
						if ($j % 2)
						{
							$trClass = "lin_par";
						...
						//------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
						//print "<td class='line'><input type='radio' id='idRadioProb' name='radio_prob' value='".$rowInfProb['prob_id']."'";
						print "<td class='line'><input type='radio' id='idRadioProb".$rowInfProb['prob_id']."' name='radio_prob' value='".$rowInfProb['prob_id']."'";
						//------------------------------------------------------------- FIM ALTERACAO --------------------------------------------------------------


							if (isset($_GET['radio_prob']) && $_GET['radio_prob'] == $rowInfProb['prob_id']) print " checked"; else
							if (isset($_GET['prob']) && $_GET['prob'] == $rowInfProb['prob_id']) print " checked"; //else

							//if (isset($_POST['radio_prob']) && $_POST['radio_prob'] == $rowInfProb['prob_id']) print " checked"; else
							//if (isset($_POST['prob']) && $_POST['prob'] == $rowInfProb['prob_id']) print " checked";
						//------------------------------------------------------------- INICIO ALTERACAO --------------------------------------------------------------
						if (isset($_GET['pathAdmin'])) //se o script estiver sendo chamado da path do módulo de administração
							print " onClick=\"ajaxFunction('divInformacaoProblema', '../../ocomon/geral/showInformacaoProb.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea' , 'radio_prob=idRadioProb".$rowInfProb['prob_id']."');\"";
						else	
							print " onClick=\"ajaxFunction('divInformacaoProblema', 'showInformacaoProb.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea' , 'radio_prob=idRadioProb".$rowInfProb['prob_id']."');\"";
						//------------------------------------------------------------- FIM ALTERACAO --------------------------------------------------------------
						print ">".$rowInfProb['problema']."</td>";

						print "<td class='line'>".NVL($rowInfProb['slas_desc'])."</td>";
						print "<td class='line'>".NVL($rowInfProb['probt1_desc'])."</td>";
						print "<td class='line'>".NVL($rowInfProb['probt2_desc'])."</td>";
						print "<td class='line'>".NVL($rowInfProb['probt3_desc'])."</td>";
